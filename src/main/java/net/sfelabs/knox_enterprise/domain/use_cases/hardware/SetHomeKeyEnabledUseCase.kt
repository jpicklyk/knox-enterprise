...
class SetHomeKeyEnabledUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (restrictionPolicy.setHomeKeyState(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set home key enabled to $params")
            )
        }
    }
}
