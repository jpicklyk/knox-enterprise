...
class SetPasswordLockDelayUseCase : WithAndroidApplicationContext, SuspendingUseCase<Int, Boolean>() {
    ...
    override suspend fun execute(params: Int): ApiResult<Boolean> {
        return when (passwordPolicy.setPasswordLockDelay(params)) {
            true -> ApiResult.Success(data = true)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set password lock delay to $params ms")
            )
        }
    }
}
