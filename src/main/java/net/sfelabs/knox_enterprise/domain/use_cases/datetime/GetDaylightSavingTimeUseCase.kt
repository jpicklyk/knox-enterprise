 ...
 */
class GetDaylightSavingTimeUseCase : WithAndroidApplicationContext, SuspendingUseCase<Unit, Boolean>() {
    private val dateTimePolicy by lazy {
        EnterpriseDeviceManager.getInstance(applicationContext).dateTimePolicy
    }

    override suspend fun execute(params: Unit): ApiResult<Boolean> {
        return ApiResult.Success(data = dateTimePolicy.daylightSavingTime)
    }
}
