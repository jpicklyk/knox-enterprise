...
class SetStatusBarIconsStateUseCase : SuspendingUseCase<Boolean, Unit>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Unit> {
        return when (val result = systemManager.setStatusBarIconsState(params)) {
            CustomDeviceManager.SUCCESS -> ApiResult.Success(Unit)
            else -> ApiResult.Error(DefaultApiError.UnexpectedError("Failed to set status bar icons state: error code $result"))
        }
    }
}
