...
class SetCellularDataEnabledUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (restrictionPolicy.setCellularData(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set cellular data enabled to $params")
            )
        }
    }
}
