...
class SetWifiStateChangeAllowedUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (wifiPolicy.setWifiStateChangeAllowed(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set WiFi state change allowed to $params")
            )
        }
    }
}
