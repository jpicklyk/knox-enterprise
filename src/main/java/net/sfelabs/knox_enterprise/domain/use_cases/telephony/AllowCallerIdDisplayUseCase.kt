...
class AllowCallerIdDisplayUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (phoneRestrictionPolicy.allowCallerIDDisplay(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(DefaultApiError.UnexpectedError("Failed to set caller ID display allowance to $params"))
        }
    }
}
