...
class GetWifiPasswordHiddenUseCase : WithAndroidApplicationContext, SuspendingUseCase<Unit, Boolean>() {
    private val wifiPolicy by lazy {
        EnterpriseDeviceManager.getInstance(applicationContext).wifiPolicy
    }

    override suspend fun execute(params: Unit): ApiResult<Boolean> {
        return ApiResult.Success(data = wifiPolicy.getPasswordHidden())
    }
}
