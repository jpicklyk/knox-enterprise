...
class SetToastEnabledStateUseCase : SuspendingUseCase<Boolean, Unit>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Unit> {
        return when (val result = systemManager.setToastEnabledState(params)) {
            CustomDeviceManager.SUCCESS -> ApiResult.Success(Unit)
            else -> ApiResult.Error(DefaultApiError.UnexpectedError("Failed to set toast enabled state: error code $result"))
        }
    }
}
