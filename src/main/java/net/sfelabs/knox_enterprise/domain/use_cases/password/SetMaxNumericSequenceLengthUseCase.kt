...
class SetMaxNumericSequenceLengthUseCase : WithAndroidApplicationContext, SuspendingUseCase<Int, Boolean>() {
    ...
    override suspend fun execute(params: Int): ApiResult<Boolean> {
        return when (passwordPolicy.setMaximumNumericSequenceLength(params)) {
            true -> ApiResult.Success(data = true)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set maximum numeric sequence length to $params")
            )
        }
    }
}
