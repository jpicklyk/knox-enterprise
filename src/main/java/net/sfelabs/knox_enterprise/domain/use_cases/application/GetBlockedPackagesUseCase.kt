...
/**
 * Use case to get all packages in the blocklist (blacklist).
 *
 * Returns a list of [AppControlInfo] containing package names and their
 * blocklist status across all administrators.
 *
 * @see AddPackageToBlocklistUseCase
 * @see RemovePackageFromBlocklistUseCase
 */
class GetBlockedPackagesUseCase : WithAndroidApplicationContext,
    ...
    override suspend fun execute(params: Unit): ApiResult<List<AppControlInfo>> {
        val blockedPackages = applicationPolicy.appPackageNamesAllBlackLists
        return ApiResult.Success(data = blockedPackages ?: emptyList())
    }
}
