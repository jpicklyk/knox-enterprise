...
class EnableSimPinLockUseCase : WithAndroidApplicationContext, SuspendingUseCase<String, Boolean>() {
    ...
    override suspend fun execute(params: String): ApiResult<Boolean> {
        val code = phoneRestrictionPolicy.enableSimPinLock(params)
        return when (code) {
            ...
            else -> ApiResult.Error(DefaultApiError.UnexpectedError("Failed to enable SIM PIN lock, status $code"))
        }
    }
}
