...
/**
 * Update an application from a local APK file.
 * @param params Absolute path to the APK file
 */
class UpdateApplicationUseCase : WithAndroidApplicationContext, SuspendingUseCase<String, Boolean>() {
    ...
    override suspend fun execute(params: String): ApiResult<Boolean> {
        return ApiResult.Success(data = applicationPolicy.updateApplication(params))
    }
}
