...
/**
 * Check if new admin activation is enabled.
 * @param params true to check current state
 */
class IsNewAdminActivationEnabledUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return ApiResult.Success(data = applicationPolicy.isNewAdminActivationEnabled(params))
    }
}
