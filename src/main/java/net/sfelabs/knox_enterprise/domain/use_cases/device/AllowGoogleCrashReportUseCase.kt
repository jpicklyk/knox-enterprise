...
class AllowGoogleCrashReportUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (restrictionPolicy.allowGoogleCrashReport(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set Google crash report allowance to $params")
            )
        }
    }
}
