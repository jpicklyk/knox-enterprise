...
class IsRoamingPushEnabledUseCase : WithAndroidApplicationContext, SuspendingUseCase<Unit, Boolean>() {
    private val roamingPolicy by lazy {
        EnterpriseDeviceManager.getInstance(applicationContext).roamingPolicy
    }

    override suspend fun execute(params: Unit): ApiResult<Boolean> {
        return ApiResult.Success(data = roamingPolicy.isRoamingPushEnabled)
    }
}
