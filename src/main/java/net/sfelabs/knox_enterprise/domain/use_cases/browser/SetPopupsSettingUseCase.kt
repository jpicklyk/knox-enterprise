...
/**
 * Set browser popups setting (allow or block popups).
 */
class SetPopupsSettingUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return ApiResult.Success(data = browserPolicy.setPopupsSetting(params))
    }
}
