...
class EnableLimitNumberOfSmsUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (phoneRestrictionPolicy.enableLimitNumberOfSms(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(DefaultApiError.UnexpectedError("Failed to set limit number of SMS to $params"))
        }
    }
}
