...
class SetMaxCharacterSequenceLengthUseCase : WithAndroidApplicationContext, SuspendingUseCase<Int, Boolean>() {
    ...
    override suspend fun execute(params: Int): ApiResult<Boolean> {
        return when (passwordPolicy.setMaximumCharacterSequenceLength(params)) {
            true -> ApiResult.Success(data = true)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set maximum character sequence length to $params")
            )
        }
    }
}
