...
class SetAutomaticWifiConnectionUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (wifiPolicy.setAutomaticConnectionToWifi(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set automatic WiFi connection to $params")
            )
        }
    }
}
