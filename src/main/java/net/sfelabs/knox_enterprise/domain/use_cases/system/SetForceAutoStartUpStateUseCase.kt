...
class SetForceAutoStartUpStateUseCase : SuspendingUseCase<Int, Unit>() {
    ...
    override suspend fun execute(params: Int): ApiResult<Unit> {
        return when (val result = systemManager.setForceAutoStartUpState(params)) {
            CustomDeviceManager.SUCCESS -> ApiResult.Success(Unit)
            else -> ApiResult.Error(DefaultApiError.UnexpectedError("Failed to set force auto startup state: error code $result"))
        }
    }
}
