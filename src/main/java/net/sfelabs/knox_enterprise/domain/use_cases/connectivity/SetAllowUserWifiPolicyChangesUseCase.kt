...
class SetAllowUserWifiPolicyChangesUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (wifiPolicy.setAllowUserPolicyChanges(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set allow user WiFi policy changes to $params")
            )
        }
    }
}
