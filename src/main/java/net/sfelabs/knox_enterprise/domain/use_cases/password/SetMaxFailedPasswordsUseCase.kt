...
class SetMaxFailedPasswordsUseCase : WithAndroidApplicationContext, SuspendingUseCase<Int, Boolean>() {
    ...
    override suspend fun execute(params: Int): ApiResult<Boolean> {
        return when (passwordPolicy.setMaximumFailedPasswordsForDeviceDisable(params)) {
            true -> ApiResult.Success(data = true)
            false -> ApiResult.Error(
                DefaultApiError.UnexpectedError("Failed to set maximum failed passwords to $params")
            )
        }
    }
}
