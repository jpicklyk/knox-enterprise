...
class AllowOutgoingSmsUseCase : WithAndroidApplicationContext, SuspendingUseCase<Boolean, Boolean>() {
    ...
    override suspend fun execute(params: Boolean): ApiResult<Boolean> {
        return when (phoneRestrictionPolicy.allowOutgoingSms(params)) {
            true -> ApiResult.Success(data = params)
            false -> ApiResult.Error(DefaultApiError.UnexpectedError("Failed to set outgoing SMS allowance to $params"))
        }
    }
}
